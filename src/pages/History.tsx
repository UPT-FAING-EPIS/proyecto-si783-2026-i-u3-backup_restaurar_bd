...
export function History() {
  ...
  const handleClearLogs = async () => {
    if (
      !window.confirm(
        "¿Estás seguro de que deseas eliminar TODOS los registros del historial? Esta acción no se puede deshacer.",
      )
    )
      return;
    try {
      ...
      alert("No se pudo limpiar el historial: " + e);
    }
  };
    ...
    <div className="h-full flex flex-col relative overflow-hidden">
      <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-6 py-4 border-b border-border flex-none gap-4">
        <div className="min-w-0">
          <h1 className="text-xl font-mono text-text truncate">
            Historial de Logs
          </h1>
          <p className="text-sm text-muted truncate">
            Registro completo de operaciones de backup y validación
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded border text-sm font-medium transition-colors
              border-error/40 text-error bg-error/10 hover:bg-error/20 disabled:opacity-30 disabled:cursor-not-allowed"
            title="Eliminar todos los registros del historial"
          >
            <Trash2 size={15} />
            Limpiar Logs
          </button>
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="border-b border-border bg-bg/50">
                <th className="px-4 py-3 text-sm font-semibold text-muted">
                  Conexión
                </th>
                <th className="px-4 py-3 text-sm font-semibold text-muted">
                  Motor
                ...
                filteredLogs.map((log) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-xs px-2 py-1 rounded border uppercase font-mono ${getEngineColor(log.engine)}`}
                      >
                        {log.engine}
                      </span>
                    </td>
            ...
            <div className="p-6 overflow-auto space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div
                    className={`font-mono text-sm font-bold ${selectedLog.status === "OK" ? "text-success" : "text-error"}`}
                  >
                    {selectedLog.status}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="text-sm font-mono flex items-center gap-2">
                    <Calendar size={14} /> {selectedLog.started_at}
                  </div>
                </div>
                <div>
                  <label className="block text-xs text-muted uppercase tracking-wider mb-1">
                  ...
                  <div className="text-sm font-mono flex items-center gap-2">
                    <Calendar size={14} /> {selectedLog.finished_at}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="font-mono text-sm">
                    {formatSize(selectedLog.file_size_bytes)} (
                    {selectedLog.file_size_bytes} bytes)
                  </div>
                </div>
                <div>
                  <label className="block text-xs text-muted uppercase tracking-wider mb-1">
                  ...
                  <div className="font-mono text-sm">
                    {selectedLog.restore_verified
                      ? "Exitosa"
                      : "Fallida / No realizada"}
                  </div>
                </div>
              </div>
