      ...
      <header className="h-16 flex items-center justify-between px-6 border-b border-border">
        <div>
          ...
          <p className="text-sm text-muted">
            Gestiona tus credenciales de base de datos
          </p>
        </div>
        <button
          onClick={() => {
            ...
            {connections.map((conn) => (
              ...
              >
                <div className="flex justify-between items-start">
                  ...
                  <span
                    className={`text-xs px-2 py-1 rounded border font-mono uppercase ${engineColors[conn.engine] || "bg-border text-muted"}`}
                  >
                    {conn.engine}
                  </span>
                </div>
                ...
                <div className="text-sm">
                  <p>
                    ...
                    <span className="text-text">{conn.database_name}</span>
                  </p>
                  <p>
                    <span className="text-muted">User:</span>{" "}
                    <span className="text-text">{conn.username}</span>
                  </p>
                </div>
                ...
                <div className="flex gap-2 mt-auto pt-4 border-t border-border">
                  <button
                    ...
                    className="flex-1 py-1.5 border border-border hover:border-accent text-muted hover:text-accent rounded flex items-center justify-center gap-2 transition-colors"
                  >
                    <Edit2 size={16} /> Editar
                  </button>
                  <button
                    onClick={() => handleDelete(conn.id)}
                    className="flex-1 py-1.5 border border-border hover:border-error text-muted hover:text-error rounded flex items-center justify-center gap-2 transition-colors"
                  >
