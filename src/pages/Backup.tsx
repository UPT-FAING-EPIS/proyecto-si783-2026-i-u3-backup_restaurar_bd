...
export function Backup() {
  ...
  const handleBackup = async () => {
    ...
    try {
      const result: any = await invoke("generate_backup", {
  ...
  const getLogColor = (level: string) => {
    switch (level) {
      ...
      case "error":
        return "text-error";
      default:
        return "text-muted hover:text-text transition-colors";
    }
  };
    ...
    <div className="h-full flex flex-col">
      <header className="h-16 flex items-center px-6 border-b border-border flex-none">
        <div>
          ...
          <p className="text-sm text-muted">
            Ejecuta volcados manuales y verifica en tiempo real
          </p>
        </div>
      </header>
      ...
      <main className="flex-1 p-6 flex flex-col gap-6 overflow-hidden">
          ...
          <div className="flex-1">
            ...
            >
              {connections.length === 0 && (
                <option value="">No hay conexiones registradas</option>
              )}
              {connections.map((c) => (
                <option key={c.id} value={c.id}>
                  {c.name} ({c.engine} - {c.database_name})
                </option>
              ))}
            </select>
          </div>
            ...
            {logs.length === 0 && status === "idle" ? (
              ...
              logs.map((log) => (
                <div key={log.id} className="flex gap-3 mb-1">
                  <span className="text-accent/70 select-none shrink-0">
                    [{log.time}]
                  </span>
        ...
        {(status === "success" || status === "error") && (
          <div
            className={`flex-none p-4 rounded-lg border flex items-center gap-3 animate-in slide-in-from-bottom-4 ${
              status === "success"
          ...
          >
            ...
            <div className="font-mono font-bold">
              {status === "success"
                ? "BACKUP VERIFICADO Y GUARDADO CORRECTAMENTE"
                : "OPERACIÓN FALLIDA — VERIFICA EL LOG PARA MÁS DETALLES"}
            </div>
          </div>
        )}
      </main>
