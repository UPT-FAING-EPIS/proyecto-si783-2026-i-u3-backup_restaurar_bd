...
export function ConnectionForm({
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!formData.database_name.trim())
      return toast.error("El nombre de la BD es obligatorio");
    if (!formData.backup_path.trim())
      return toast.error("Debes seleccionar una carpeta de backup");
    if (formData.port <= 0 || formData.port > 65535)
      return toast.error("Puerto inválido");
      ...
      <div className="w-[450px] bg-surface h-full border-l border-border flex flex-col shadow-2xl animate-in slide-in-from-right">
        ...
        >
          ...
          <div>
            ...
            <select
              className="w-full bg-bg border border-border rounded px-3 py-2 text-text outline-none focus:border-accent transition-colors"
          ...
          <div className="flex gap-4">
            ...
            <div className="flex-1">
              <label className="block text-sm text-muted mb-1">
                Contraseña{" "}
                {formData.id ? "(Dejar en blanco para no cambiar)" : "*"}
              </label>
              <input
                type="password"
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                required
                type="text"
                className="flex-1 bg-bg border border-border rounded px-3 py-2 text-text text-sm"
                value={formData.backup_path}
                onChange={(e) =>
                  setFormData({ ...formData, backup_path: e.target.value })
                }
                placeholder="Selecciona o escribe una ruta..."
              />
              <button
                type="button"
                onClick={selectFolder}
                className="bg-border hover:bg-muted/30 px-3 rounded flex items-center justify-center"
              >
                <Folder size={18} />
              </button>
            </div>
          ...
          <div className="flex items-center justify-between p-3 bg-bg border border-border rounded">
            <div className="flex items-center gap-2">
              <Cloud
                size={18}
                className={formData.use_ssl ? "text-accent" : "text-muted"}
              />
              <div>
                <span className="text-sm text-text">
                  Conexión en la nube (SSL)
                </span>
                <p className="text-xs text-muted mt-0.5">
                  Activar para Railway, AWS RDS, PlanetScale, etc.
                </p>
              </div>
            </div>
            <button
              type="button"
              onClick={() =>
                setFormData({ ...formData, use_ssl: !formData.use_ssl })
              }
              className={`relative w-11 h-6 rounded-full transition-colors duration-200 ${
                formData.use_ssl ? "bg-accent" : "bg-border"
              }`}
            >
              <span
                className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full transition-transform duration-200 ${
                  formData.use_ssl ? "translate-x-5" : "translate-x-0"
                }`}
              />
            </button>
          </div>
          ...
          <div className="pt-4 border-t border-border mt-6">
            ...
            >
              {testResult === "testing" ? (
                <Zap size={18} className="animate-pulse text-accent" />
              ) : (
                <Database size={18} />
              )}
        ...
        <div className="p-6 border-t border-border flex gap-4">
          <button
            onClick={onClose}
            className="flex-1 py-2 text-muted hover:text-text"
          >
            Cancelar
          </button>
          <button
            onClick={handleSave}
            className="flex-1 bg-accent text-bg font-semibold rounded hover:bg-accent/90 transition-colors"
          >
            Guardar
          </button>
        </div>
