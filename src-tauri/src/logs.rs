...
pub fn get_dashboard_stats(state: State<'_, AppState>) -> Result<DashboardStats, String> {
    ...
    let successful_backups: i64 = db
        .query_row(
            "SELECT COUNT(*) FROM backup_logs WHERE status = 'OK'",
            [],
            |row| row.get(0),
        )
        .unwrap_or(0);

    let failed_backups: i64 = db
        .query_row(
            "SELECT COUNT(*) FROM backup_logs WHERE status = 'FAIL'",
            [],
            |row| row.get(0),
        )
        .unwrap_or(0);

    let total_bytes_backed_up: i64 = db
        .query_row(
            "SELECT SUM(file_size_bytes) FROM backup_logs WHERE status = 'OK'",
            [],
            |row| row.get(0),
        )
        .unwrap_or(0);
...
pub fn clear_logs(state: State<'_, AppState>) -> Result<(), String> {
    ...
    db.execute("DELETE FROM backup_logs", [])
        .map_err(|e| format!("Error al limpiar logs: {}", e))?;
    Ok(())
}
